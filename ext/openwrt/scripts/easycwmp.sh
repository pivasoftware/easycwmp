...
handle_action() {
    ...
    if [ "$action" = "apply_download" ]; then
        if [ "$__arg1" = "3 Vendor Configuration File" ]; then 
            ...
            local minute=`date '+%M'`
            local next_minute=$(( $minute + 1))
            local tmp=`date '+%H %d %m'`
            #can not run sysupgrade now, should put it into cron
            (crontab -l; echo "$next_minute $tmp * sysupgrade $DOWNLOAD_FILE; reboot") | crontab -
            fault_code="$?"
            if [ "$fault_code" != "0" ]; then
                    let fault_code=$E_DOWNLOAD_FAIL_FILE_CORRUPTED+9000
                    easycwmp_status_output "" "$fault_code"
